...
VLIB_REGISTER_NODE(sf_ip4_reass_node) = {
    .name = "sf_ip4_reass",
...
static_always_inline void ip4_frag_build_pkt(vlib_main_t *vm , vlib_buffer_t **ret_buffer , int32_t *ret_status)
{
    uint16_t frag_offset;
    uint16_t frag_size;
    uint16_t ip4_hdr_len;
    uint16_t ip4_hdr_len_tmp;
    ...
    // sf_wqe_advance(ip_reass_b0 , sf_wqe_get_current_offset(tmp) );

    //copy ip4 head
    ip4_hdr_len = ip_reass_record(tmp)->ip_hdr_len;
    // sf_debug("from %p to %p , len %d\n" , sf_wqe_get_current(tmp) , sf_wqe_get_current(ip_reass_b0) , ip4_hdr_len);
    memcpy(sf_wqe_get_current(ip_reass_b0) , sf_wqe_get_current(tmp) , ip4_hdr_len);
    ip4_hdr = (ipv4_header_t *)sf_wqe_get_current(ip_reass_b0);
    ip4_hdr->flags = 0;
...
#endif
    sf_wqe_advance(ip_reass_b0 , ip4_hdr_len );
    ...
    while(tmp!= NULL)
    {
        ip4_hdr_len_tmp = ip_reass_record(tmp)->ip_hdr_len;
        ...
        // sf_debug("from %p to %p , len %d\n" , sf_wqe_get_current(tmp) + ip4_hdr_len_tmp , data_ptr + frag_offset , frag_size);
        memcpy(data_ptr + frag_offset , sf_wqe_get_current(tmp) + ip4_hdr_len_tmp , frag_size );
    ...
    //  // reset to ip hdr
    // sf_wqe_advance(ip_reass_b0 , -ip4_hdr_len);
    // new_wdata->ip_wdata_cnt --; // we will go to ip4_input later
...
//             -1 : pre l3 layer is larger than ip reass layer config
static_always_inline void ip4_frag_proc(
    vlib_main_t *vm,
...
)
{
    ...
    *ret_status = 0;
    ...
    // sf_debug("ip_reass_output : %d\n" , ip_reass_rec->ip_reass_output);
    ...
    key.sip.addr_v4   = ip4_hdr->src_ip;
// sf_debug("sip : 0x%x\n" , ip4_hdr->src_ip );
    key.dip.addr_v4   = ip4_hdr->dst_ip;
// sf_debug("dip : 0x%x\n" , ip4_hdr->dst_ip );

    key.ip_id   = ip4_hdr->id;
// sf_debug("ip_id : 0x%x\n" , ip4_hdr->id );
...
// sf_debug("hash_index : %x\n" , hash_index);
    ...
    if ( pnode )
    {
        ...
        if( (get_ip4_hdr_flag(ip4_hdr) & IPV4_FLAGS_MORE_FRAG_MASK) == 0  )
        {
        ...
        if(ip_reass_data->meet_len == ip_reass_data->total_len)
        {
            ...
            *ret_status = ip_reass_data->cache_pkt_num;
            ...
            ip4_frag_build_pkt(vm , ret_buffer , ret_status);
...
VLIB_NODE_FN(sf_ip4_reass_node)
(vlib_main_t *vm,
           ...
           vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0)
    {
        ...
        ip4_frag_proc(vm , &b0 ,  &ret_status , worker_index);
