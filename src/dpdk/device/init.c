...
/************ SF start ************/
	...
	{
		devconf->num_rx_queues = vlib_num_workers();
	}
/************ SF end ************/
...
#undef _
  ...
  if (ret < 0)
    return clib_error_return (0, "rte_eal_init returned %d(%s)", ret , rte_strerror(rte_errno));
...
/*** SF start ***/
int sf_nports = pool_elts (vnm->interface_main.sw_interfaces) - 1;

vlib_node_t *update_intf_phy_stat_node = vlib_get_node_by_name(vm , (u8 *)"update_intf_phy_stat");

if(update_intf_phy_stat_node == NULL)
{
	clib_error("no node name update_intf_phy_stat\n");
}

if(sf_nports > MAX_INTERFACE_CNT)
{
	clib_error("[Fatal] too much interfaces !!!!!!!!\n");
}

if(interfaces_stat != NULL)
{
	interfaces_stat->dpdk_port_cnt = sf_nports;
  ...
  vec_foreach (xd, dm->devices)
  {
    struct rte_eth_dev_info di;
  	rte_eth_dev_info_get (xd->port_id, &di);
		if(vnet_get_sw_interface_safe(vnm , xd->sw_if_index) != NULL)
		{
			interfaces_stat->dpdk_used_queue[xd->sw_if_index - 1] = xd->rx_q_used;
			interfaces_stat->dpdk_max_queue[xd->sw_if_index - 1] = di.max_rx_queues;
		}
...
{
	clib_error("interfaces_stat is NULL");
}

sf_main_stat->sf_vpp_status = 0;

// notify update_intf_phy_stat to init dpdk ports
vlib_process_signal_event(vm , update_intf_phy_stat_node->index , 1 , 1); // "1 ,1" has no meaning
/*** SF end ***/
